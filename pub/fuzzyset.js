...
window.FuzzySet = function(arr, useLevenshtein, gramSizeLower, gramSizeUpper) {
	...
	var _iterateGrams = function(value, gramSize) {
		...
		var simplified = '-' + value.toLowerCase().replace(_nonWordRe, '') + '-',
			...
			results = [];
		if (lenDiff > 0) for (var i = 0; i < lenDiff; ++i) {
			value += '-';
		}
	...
	fuzzyset._get = function(value) {
		var normalizedValue = this._normalizeStr(value),
			results = this._exact(normalizedValue);
	...
	fuzzyset._exact = function(value) {
		var result = this.exactSet[value];
		if (result) return [[1, result]];
		var results = [];
		for (target in this.exactSet) {
			var position = target.indexOf(value);
			if (position!==-1)
				results.push([1-position/target.length, this.exactSet[target]]);
		}
		if (results.length) return results.sort(sortDescending);
	 }
		...
		for (gram in gramCounts) {
			...
			if (gram in this.matchDict) {
				for (i = 0; i < this.matchDict[gram].length; ++i) {
					...
					if (index in matches) matches[index] += gramCount * otherGramCount;
					else matches[index] = gramCount * otherGramCount;
				}
			}
		}
